  ...
  <view class="journey_ownership_warp">
    ...
    <view class='journey_ownership' :style="ishowbuilding?'padding-bottom:97rpx':'background:transparent'" v-else @click.stop="handleBodyClick">
      ...
      <view v-if="ishowbuilding">
        <swiper :style="{height:swiperHeight}" class="swiper" :current='curr' :next-margin='swiperMargin' :previous-margin='swiperMargin' :indicator-dots="indicatorDots" :autoplay="autoplay"
          :circular='autoplay' @change="changeSwipe">
                  ...
                  <view class="list_item_warp" @click="toDetail(item.reportBuildingIntro.buildingId,item.reportBuildingIntro.userId,item.reportData,item.ishowPlanStatus)">
                    ...
                    <view class="item_details">
                      <view class="details_title">
                        ...
                        <view class="label cl_Y" v-if="!!item.reportBuildingIntro.propertyType">
                          {{handlePropertyType(item.reportBuildingIntro.propertyType.length>0&&item.reportBuildingIntro.propertyType!=null?item.reportBuildingIntro.propertyType[0]:'')}}</view>
                      </view>
                      ...
                      <view class="address_details"><text>{{item.reportBuildingIntro.areaName}}<text v-if="item.reportBuildingIntro.streetName">-</text>{{item.reportBuildingIntro.streetName}}</text>
                      </view>
                      <view class="classify">
                        <view class="claWarp">
                  ...
                  <view class='demand' v-if="item.ishowPlanStatus">
                    ...
                    <view class="recommend_warp" v-if="JSON.stringify(item.recommendation) != '{}'&&item.recommendation!=null">
                      ...
                      <view v-if="item.recommendation!=null&&JSON.stringify(item.recommendation) != '[]'">
                        ...
                        <u-tabs name='tableTitle' :list="item.recommendation" :is-scroll="true" :current="currentPlan" @change="changePlanTab" active-color="#062471" inactive-color="#999999"
                          font-size="30" bar-width='40' bar-height='6' bg-color='transparent'>
                        </u-tabs>
                          ...
                          <swiper-item v-for="(itemR,index) in item.recommendation" :key="index">
                            <view :class="'change_box_warp'+index">
                              <view class="change_box">
                                ...
                                <view class="rows">
                                  <i class="iconfont icongeju"></i><text class="lable">格局</text><text
                            ...
                            <view class="list_top_title">
                              ...
                              <view class="keeper_msg" :id="itemL.userId" @click="e=>tohouseKeeper(e,itemL)">
                                ...
                                <view class="keeper_name">
                                  <text class="keeper_name_text">{{itemL.userName?itemL.userName:'--'}}</text> <i class="iconfont icondianhua" :id="itemL.userId"
      ...
      getData('/dt-business/home/cAppHome', params)
        .then((res) => {
          ...
          res.forEach((itemR, index) => {
            ...
            //判断是否有方案推荐
            if (
              JSON.stringify(itemR.customerIntention) == '{}' &&
              itemR.customerIntention == null &&
