	...
	export default {
		...
		methods: {
			...
			getBuildingInfo(baseInfo){
				...
				baseInfo.openTime = baseInfo.openTime?this.$tool.DataFormatIos(baseInfo.openTime) :'开盘待定'
			...
			getDynamicAndDate(openTimeList,dynamicList) {
				...
				let dateBaseInfoList =(openTimeList.list||[]).map(item=>{
					item.evidenceDate = item.evidenceDate ? self.$tool.DataFormatIos(item.evidenceDate) : '';
					item.identifyChipsDate = item.identifyChipsDate ? self.$tool.DataFormatIos(item.identifyChipsDate) : '';
