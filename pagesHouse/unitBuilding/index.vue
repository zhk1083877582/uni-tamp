		...
		<view class="movable_warp">
			<movable-area class="M-movable-area">
				<movable-view class="top_img_warp" direction="all" scale="true" scale-min="1" scale-max="4" scale-value="1">
					<image class="top_img" :src="TopSrc+'?x-oss-process=image/resize,h_267,w_375'"></image>
					<text  v-for="(item,index) in UnitList" :key='index' @click="handleclickUnit(item,index)" :class="[item.unitName==current?'crrent_img_btn':'img_btn']" :style="{position:'absolute','top':item.topMargin*2+'rpx','left':item.leftMargin*2+'rpx'}">{{item.unitName}}#</text>
				</movable-view>
			</movable-area>
		</view>
			...
			<view class="main_wap">
				<view class="main" v-for="(item,index) in oneRoomAndOnePrice" :key='index' v-show="item.typeFlag == 1" @click="clickHouseType(item)">
					<view class="top_img">
						...
						<image class="img" :src="item.annexPath+'?x-oss-process=image/resize,h_200,w_200'"></image>
...
export default {
	...
	methods: {
		...
		tagclick(obj,index){
			...
			console.log(this.buildingId,this.unitCrrentObj.houseUnitElement,this.crrentObj.id)
			this.getUnitData(this.buildingId,this.unitCrrentObj.houseUnitElement,this.crrentObj.id,this.unitCrrentObj.typeIds);
		},
		//点击户型
		clickHouseType(obj){
			console.log(obj)
		...
		getUnitList(buildingId){
			...
			getData("/building/capp/buildingUnit/buildingUnitList",param).then(res=>{
		...
		getUnitData(buildingId,houseUnitElement,id,typeIds=''){
			...
			getData("/building/unit/house/oneRoomAndOnePrice",param).then(res=>{
				if(typeIds){
					let arr = []
			...
			.main{
				...
				.top_img{
					...
					.image_text{
						...
						color: #FFFFFF;
						line-height: 48rpx;
						background: linear-gradient(180deg,rgba(0,0,0,0), rgba(0,0,0,0.81) 99%);
						position: absolute;
						bottom: 0;
						width: 100%;
						text-align: center;
