...
export default {
  ...
  methods: {
    ...
    doFormatImgList(annexs) {
      ...
      this.ImgStart =
        this.mp4Num > 0 || this.VRNum > 0 ? this.mp4Num + this.VRNum : 0
    },
    //轮播图切换
    ...
    doFormatAveragePrice(type, averagePriceMin, averagePriceMax) {
      ...
      if (!averagePriceMin) {
        ...
        if (type == 1) {
          ...
          return `${this.unitChange(averagePriceMin)}-${this.unitChange(
            averagePriceMax
          )}万元/㎡`
        } else {
