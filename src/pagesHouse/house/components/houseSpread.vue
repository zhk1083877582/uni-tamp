    ...
    <view class="movable_warp">
      <movable-area class="M-movable-area">
        <movable-view class="top_img_warp" direction="horizontal" scale="true" scale-min="1" scale-max="4" scale-value="1">
          <image class="top_img" :src="spreadAnnexPath?spreadAnnexPath:spreadAnnexPath"></image>
          <text v-for="(item,index) in buildingUnitBaseInfoList" :key='index' @click="handleclickUnit(item,index)" :class="[item.numberName==current?'crrent_img_btn':'img_btn']"
...
export default {
  ...
  mounted() {
    let self = this
    self.current =
      self.buildingUnitBaseInfoList[0] &&
      self.buildingUnitBaseInfoList[0].numberName
