  ...
  <view class="houseTypeDetailsAll">
    <view class="swiperBox" :class="{'swiperBox1': tanFlag}">
      ...
      <view class="custom-indicator" v-if="!tanFlag">
        ...
        <text class="biao" @click="spanSwipeClick(item)" :class="{'active': item.type == currentType}" v-for="(item, index) in houseBan1" :key="index">
        ...
        <view class="typeListCont">
          <view class="typeListContLi" v-for="item in houseInfo.houseTypeListVOS" :key="item.id">
            <view class="aBox" @click="goHouseTypeDetails(item.id)">
              <view class="left">
                <image class="leftImg" :src="item.annexPath"></image>
...
export default {
  ...
  methods: {
    linkJump() {
      ...
      uni.navigateTo({
        url:
          '../webView/webView?toMWebpath=' +
          path +
          '&loanTotal=' +
          (this.houseInfo.totalPrice * 0.65).toFixed(2),
      })
    },
    buildingClick() {
      uni.navigateTo({
    ...
    goHouseTypeDetails(val) {
      ...
      uni.navigateTo({
        url:
          '/pagesHouse/houseTypeDetails/houseTypeDetails?buildingId=' +
          this.buildingId +
          '&&houseTypeId=' +
          val,
      })
        ...
        .then((res) => {
          ...
          if (this.houseInfo.houseTypeTag) {
            if (this.houseInfo.houseTypeTag.indexOf(',') == -1) {
              ...
              this.houseInfo.houseTypeTag =
                this.houseInfo.houseTypeTag.split(',')
            }
          }
          ...
          this.houseBan1 = this.houseBan.reduce(
            (all, next) =>
              all.some((item) => item['type'] == next['type'])
                ? all
                : [...all, next],
            []
          )
