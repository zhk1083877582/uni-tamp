<template>
  <view class="typeList">
    <!-- #ifndef MP-BAIDU -->
    <view class="bedroom">
      <u-tabs name="bedroom" :list="tabList" :current="current" @change="tabsChange1" font-size='30' bar-height="4" bar-width="34" inactive-color="#A9A8AE" active-color="#00a4ff"></u-tabs>
    </view>
    <!-- #endif -->
      ...
      <view class="bedContainer" v-for="item in houseList" :key="item.id">
        <view class='bedBox' @click="goHouseTypeDetails(item.id)">
          <view class="bedLeft">
            ...
            <view class="leftImg">
              <image class="typeListImg" :src="item.annexPath"></image>
            </view>
...
export default {
  ...
  methods: {
    ...
    goHouseTypeDetails(val) {
      uni.navigateTo({
        url:
          '../houseTypeDetails/houseTypeDetails?buildingId=' +
          this.buildingId +
          '&&houseTypeId=' +
          val,
      })
