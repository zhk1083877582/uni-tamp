    ...
    <view class="main-mode">
      <view class="item" v-for="(item,index) in majordomoList" :key="index">
        <view class="major-mode" @click="tochathouseKeeper(item)">
          <image :src="item.avatarUrl ? item.avatarUrl :imgText" class="user-icon"></image>
...
export default {
  ...
  methods: {
    ...
    tochathouseKeeper(obj) {
      ...
      uni.navigateTo({
        url:
          '/pagesChat/chatDetail/houseKeeper?id=' +
          imBaseUserId +
          '&title=' +
          userName,
      })
    ...
    getmajordomo() {
      ...
      getData('/tospurWeb/v1/capp/rongcloud/more/getHousekeeperList', params)
        .then((res) => {
